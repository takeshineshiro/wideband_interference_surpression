%%%%%%%%????????????????????????-??????????????????????????????????????????%%%%%%%%%%%%%%%
   ...
   angle_vector_2  = [];
     ...
     angle_matrix      =  [angle_vector_wei, angle_vector_0,angle_vector_1, angle_vector_2];
      ...
      %%%%%%%%%%%%%%%%%%%%%%%%%IIR%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
       ...
       xx_ad_in   =  angle_matrix*ss_matrix+vv_vector ;                   % ????????????????????????
 ...
 %%%%%%%%%%%%%%%%simple  ierator %%%%%%%
      ...
      for   i  =  1:size(xx_vector,2)
          
          if(i==1)
                  yy(:,i)  = cc_ini*xx_vector(:,i);
                  mu(i)    = xx_vector(:,i)'*yy(:,i);
                  gg(:,i)  = (1/(1+mu(i)))*yy(:,i);
                  cc       = (eye(M)-gg(:,i)*xx_ini)*cc_ini;
                  
          else
                 yy(:,i)   = cc*xx_vector(:,i);
                 mu(i)     = xx_vector(:,i)'*yy(:,i);
                 gg(:,i)   = (1/(1+mu(i)))*yy(:,i);
                 cc        =  (eye(M)-gg(:,i)*xx_vector(:,i-1)')*cc;
      ...
      w_opt     = cc*f_theta0/(f_theta0'*cc*f_theta0);
