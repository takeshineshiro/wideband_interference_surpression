 ...
 y_index  = R*[1,1,-1,-1];
  ...
  %%%%%%%%%%%%%%%%%%%base_band%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   ...
   for   i  = 1:size(ss_matrix,2)-N_snapshot
    ...
    %    xx_node  =  xx_vector(i,:);
    %    xx_rr    =  xx_node*xx_node';     % rr_x =sum(x(t)*x(t)^T)/snapshot  
    %    xx_rr_av(i)  = xx_rr/N_snapshot;
    %    xx_rr_martix = [xx_rr_martix,xx_rr_av(i)];
    %    end
    
    
    
    
    xx_rr   =   xx_vector*xx_vector';                           %rr_x =sum(x(t)*x(t)^T)/snapshot  
    
    xx_rr_av   =   xx_rr/N_snapshot;
    
    
    inv_xx_rr   =  inv(xx_rr_av);                               % inv
    ...
    w_opt       = inv_xx_rr*f_theta0/(f_theta0'*inv_xx_rr*f_theta0);     % w_opt  = R^-1*f(theta)/(f^-1(theta)*R^-1*f(theta))
    
    
    
     for     i  = 1:N_snapshot                                            % SMI_PI
