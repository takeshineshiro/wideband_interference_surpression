%%%%%%%%????????????????????????-??????????????????????????????????????????%%%%%%%%%%%%%%%
   ...
   angle_vector_2  = [];
     ...
     angle_matrix      =  [angle_vector_wei, angle_vector_0,angle_vector_1, angle_vector_2];
      ...
      for  i   = 1:N_snapshot
           ...
           xx_vector(:,i)  =angle_matrix*ss_matrix(:,i)+vv_vector(:,i);
 ...
 %%%%%%%%%%%%%%%%simple  ierator   stap %%%%%%%
      ...
      for  i   =  1:N_snapshot-P+1
          xx_block  = xx_vector(:,i:i+P-1);
          xx_seq    = [];
          
          
      for   k =  1:size(xx_block,2)
            xx_temp  = xx_block(:,k);
            xx_seq   = [xx_seq;xx_temp];
      end
            xx_comb  = [xx_comb,xx_seq];
      
          if(i==1)
                  yy(:,i)  = cc_ini*xx_comb(:,i);
                  mu(i)    = xx_comb(:,i)'*yy(:,i);
                  gg(:,i)  = (1/(1+mu(i)))*yy(:,i);
                  cc       = (eye(M*P)-gg(:,i)*xx_ini)*cc_ini;
                  
          else
                 yy(:,i)   = cc*xx_comb(:,i);
                 mu(i)     = xx_comb(:,i)'*yy(:,i);
                 gg(:,i)   = (1/(1+mu(i)))*yy(:,i);
                 cc        =  (eye(M*P)-gg(:,i)*xx_comb(:,i-1)')*cc;
        ...
        a_s1   =  [1,1,1,1]'   ;                                      % constrain  anna , no time domain  constraint
        a_t1   =   [1,0,0,0,0]';
                  
        a_st_1 =   kron(a_s1,a_t1);                                   %  a_st
        
        w_st   =    cc*a_st_1/(a_st_1'*cc*a_st_1);                    %  w_st
